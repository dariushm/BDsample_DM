...
//   plist.nspec_complex=ncomplex;
  ...
  */
  ...
  deltheta=2.0/(1.0*(Nthet-1));//Here we use -1 because we are not integrating.
  ...
  for(i=0;i<Ndih;i++){
    dihvec[i]=plist.dihedstart+i*deldih;
  }
  ...
  */
  ...
  for(s=0;s<Nx0;s++){
    ...
    if(rank==0){
      ...
      sprintf(pname, "prr_x0_%g_dh%g_dt%g.dat", x0, plist.dihedstart, deltat_reac);
      prrfile.open(pname);
      
      sprintf(pname, "pav_x0_%g_dh%g_dt%g.dat", x0, plist.dihedstart, deltat_reac);
      pavfile.open(pname);
      pavfile<<0<<'\t'<<0<<'\t'<<0<<'\t';
      for(i=0;i<Rbins;i++){
	...
	for(hh=0;hh<Ndihsim;hh++){
	  ...
	  for(dd=0;dd<Npsibins;dd++){
	    ...
	    if(bothperp==1){
	      ...
	      if(rank==0){
		sprintf(tname, "prt_sprob_x0_%g_dh0_%g_cat%g_cbt%g_PHI%g_dt%g.dat",x0, dihed,  costheta[ct], costheta[bt], phi2, deltat_reac);
		rfile.open(tname);
		sprintf(tname, "St_x0_%g_dh%g_ct%g_c2t_%g_PHI%g_dt%g.dat",x0, dihed, costheta[ct], costheta[bt], phi2, deltat_reac);
		probfile.open(tname);
	      }
	    }else{
	      if(rank==0){
