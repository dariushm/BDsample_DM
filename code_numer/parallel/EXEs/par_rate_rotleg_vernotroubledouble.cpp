...
int main(int argc, char *argv[])
{
  ...
  if((Nitbin-1)==Nwrite*statwrite*1.0)
    Nitbin-=1;
	  ...
	  */
	  ...
	  for(rep=0;rep<myNrep;rep++){
		...
		*/
		...
		itmove=floor(deltat/deltat_reac);
		...
		itmax=Nitbin-it;
	      ...
	      for(n=1;n<itmax;n++){
		if(std::fmod((it+n), statwrite*1.0)<1){
		  ind=int((it+n)/(statwrite*1.0));
		  phist[ind]+=1;
		}
	      }    
...
// 	    probfile<<0<<' '<<1<<' '<<1<<endl;
      ...
      for(di=statwrite*1.0;di<Nitbin;di+=statwrite*1.0){
		tval=floor(di)*deltat_reac;
		ind=int(floor(di)/(statwrite*1.0));
