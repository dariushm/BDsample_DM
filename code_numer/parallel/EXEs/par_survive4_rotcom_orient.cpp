...
int main(int argc, char *argv[])
{
  ...
  if(rank==0){
    ...
    for(i=0;i<plist.Nthet;i++){
      dubparm[11+i]=plist.ctlist[i];
    }
...
//   plist.nspec_complex=ncomplex;
  ...
  */
  ...
  cout <<"rank: "<<rank<<"Ntheta: "<<Nthet<<'\t';
  double *costheta=new double[Nthet];
  for(i=0;i<Nthet;i++){
    costheta[i]=plist.ctlist[i];
    
    cout <<"rank: "<<rank<<"costheta: "<<costheta[i]<<endl;
  }
  deltheta=costheta[i-2]-costheta[i-1];
  cout <<" delthet: "<<deltheta<<endl;
