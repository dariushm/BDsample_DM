...
int main(int argc, char *argv[])
{
  ...
  double dudr;
  double Fx, Fy, Fz;
  double rc=plist.vr_rc;
  cout <<"BetaU(r) rc factor: "<<rc<<endl;
  int cnt;
  double Rrange=3.5*sqrt(6.0*Dtot*Maxtime)+plist.x0-bindrad[0];
  ...
  int rind;
  double *prhist=new double[1000];//maxRbins
  double *prhist_sum=new double[1000];
  ofstream rfile;
  

  for(s=0;s<Nx0;s++){
    x0=x0vec[s];
    Rrange=3.5*sqrt(6.0*Dtot*Maxtime)+x0-bindrad[0];
    delR=0.01;
    Rbins=int(Rrange/delR);
    if(Rbins>1000){
      Rbins=1000;
      delR=(Rrange)/(1.0*Rbins);
    }
      ...
      while(bases[0].nfree>0 &&currtime<Maxtime)
	{
	  ...
	  if(R1<Rmax){
	    ...
	    if(prob<rnum){
	      ...
	      dudr=-rc/(R2);
	      Fx=-dudr*bases[1].xcom/R1;
	      Fy=-dudr*bases[1].ycom/R1;
	      Fz=-dudr*bases[1].zcom/R1;
	      dx=sqrt(2.0*deltat*wholep[1].Dx)*GaussV()+wholep[1].Dx*deltat*Fx;
	      dy=sqrt(2.0*deltat*wholep[1].Dy)*GaussV()+wholep[1].Dy*deltat*Fy;
	      dz=sqrt(2.0*deltat*wholep[1].Dz)*GaussV()+wholep[1].Dz*deltat*Fz;
	    ...
	    */
	    ...
	    deltat=deltat_reac+scaled*0.1*(R1-Rmax)*(R1-Rmax)/(Dtot*2.0);//+deltat_reac;
