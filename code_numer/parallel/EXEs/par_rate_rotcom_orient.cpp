...
//   plist.nspec_complex=ncomplex;
  ...
  cout <<"rank: "<<rank<<"Leg length 1: "<<leglen1<<endl;
  double leglen0=plist.leglen0;
  double leglen0_sq=leglen0*leglen0;
  cout <<"rank: "<<rank<<"Leg length 2: "<<leglen0<<endl;
  double bindrad=leglen1+leglen0;
  if(bindrad!=plist.bindrad){
    cout <<"SUM OF LEGS IS NOT EQUAL TO BINDING RADIUS!! "<<endl;
    exit(1);
  }
  double T=293;//K
  ...
  if(plist.nuflag<0){
    cout <<"Setting hydrodynamic radius from D and Stokes-Einstein. "<<endl;
    crad1=preT/(plist.D*Dfrac);//cut D in half because it is Da+Db
    crad0=preT/(plist.D*(1.0-Dfrac));//cut D in half because it is Da+Db
  }else{
    cout <<"Setting hydrodynamics radius from LEGLENGTH!, CALCULATING VISCOSITY FROM STOKES EINSTEIN! "<<endl;
    crad1=plist.leglen1;
    crad0=plist.leglen0;
    nu=kb*T/(6.0*M_PI*crad1*D1)*1E27/1E6;
    preR=kb*T/(8.0*M_PI*nu)*1E27/1E6;//1E27 is nm^3 and 1E6 is us
    cout <<"NEW VISCOSITY: "<<nu<<" D1 (protein1:) "<<D1<<" Dfraction1: "<<Dfrac<<" radius: "<<crad1<<" preR: "<<preR<<" Drot1: "<<preR/(crad1*crad1*crad1)<<endl;
  }
  ...
  cout <<"rank: "<<rank<<"D: "<<plist.D <<" effective radius, pro1: "<<crad1<<" D, calc, total: "<<wholep[1].Dx<<" Drot, calc: "<<wholep[1].Drx<<endl;
  wholep[0].Dx=0;
  wholep[0].Dy=0;
  wholep[0].Dz=0;
  ...
  cout <<"rank: "<<rank<<"D: "<<plist.D <<" effective radius, pro0: "<<crad0<<" D, calc (unmoving): "<<wholep[0].Dx<<" Drot, calc: "<<wholep[0].Drx<<endl;
  double r1x, r1y, r1z;
  double cthet1;
  ...
  cout <<"rank: "<<rank<<" kappa: "<<kappa<<" kappa*4*pi*sigma^2: "<<kappa*4.0*M_PI*bindrad*bindrad<<endl;
  ...
  */
  ...
  cout <<"Delpsi: "<<delpsi<<endl;  
  double *psivec=new double[Npsibins];
  for(i=0;i<Npsibins;i++){
    psivec[i]=-psi_limit+i*delpsi;
  ...
  */
  ...
  for(s=0;s<Nx0;s++){
	...
	for(hh=0;hh<Ndihsim;hh++){
	  ...
	  lz_0=+costheta[bt]*leglen0;//switched from other program
	  ...
	  for(dd=0;dd<Npsibins;dd++){
		...
		*/
		...
		if(bothperp==0)
		  psimol=set_norm_to_psi(psi0, bases, leglen1, leglen0, molfix, molrot, c1toim);
		else{
		  /*in this case, only two psis are sampled, rotate phi2 and calc what psi*/
		...
		*/
	    ...
	    sprintf(tname, "St_x0_%g_dh%g_ct%g_c2t_%g_psi%g_dt%g.dat",x0, dihed, costheta[ct], costheta[bt], psi0, deltat_reac);
	    ...
	    if(rank==0){
	      ...
	      //probfile<<0<<' '<<1<<' '<<1<<endl;
	      //afile<<x0<<'\t';
	      totfullnorm+=norm_psi[dd]*delpsi*deltheta*deltheta*dihnorm;
		...
		//cout <<"Time: "<<tval<<" S(t): "<<stot[ind]<<endl;
	      ...
	      sonlyfile <<dihed<<'\t'<<costheta[ct]<<'\t'<<costheta[bt]<<'\t'<<psi0<<'\t'<<phist_sum[Nwrite-1]/(1.0*nprocs*norm_psi[dd])<<'\t'<<norm_psi[dd]<<endl;
    ...
    */
    ...
    if(rank==0){
      cout <<"Total norm fact : "<<totfullnorm<<" equal to ?: "<<M_PI*2.0*M_PI*anglelimit*anglelimit*2.0*psi_limit<<endl;
      ratefile.open(tname);
      for(i=statwrite;i<Nitbin;i+=statwrite){
	tval=i*deltat_reac;
	ind=i/statwrite;
