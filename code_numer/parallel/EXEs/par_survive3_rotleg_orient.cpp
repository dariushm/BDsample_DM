...
//   plist.nspec_complex=ncomplex;
  ...
  double rotrad=plist.rotrad;
  double leglen=bindrad[0]/2.0;//plist.leglen;
  double leglen2=leglen*leglen;
  cout <<"rank: "<<rank<<"Leg length: "<<leglen<<" rotation radius: "<<rotrad<<endl;
  double T=293;//K
  ...
  */
  ...
  double delpsi=2*M_PI/(1.0*(Npsibins-1));
	  ...
	  for(hh=0;hh<Ndihsim;hh++){
		...
		*/
		...
		if(bothperp==0)
	      ...
	      /*Now that we have the starting orientations, we need to add the rotation site. 
		the jacobian will have to be measured via rotation around that site
	      */
