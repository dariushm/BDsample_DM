...
and symmetrized form of equation (e.g. p is scaled by peq^1/2)
 
also calculates flux
oct 2010
maggie johnson
*/
...
int main(int argc, char *argv[])
{
  ...
  for(t=1;t<Nit;t++){
	...
	pnew[m*N+n]=p[m*N+n]+dt/hx2*Dxx*(p[(m+1)*N+n]+p[(m-1)*N+n]-2*p[m*N+n]);//first add in second derivative points at the old values
