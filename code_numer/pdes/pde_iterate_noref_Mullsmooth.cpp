...
created more boundaries that are not square to avoid including the very steep 
part of the potential into the problem, it varies too rapidly and induces
oscillations given the size of the time step
oct 2010
maggie johnson
*/
...
double calc_ygradient(double x, double y);
double calc_xgradient(double x, double y);
...
int main(int argc, char *argv[])
{
  ...
  Dyy=1.0/beta;
  dDy=0;
  cout <<"Dxx: "<<Dxx<<" Dyy: "<<Dyy<<" dDx: "<<dDx<<" dDy: "<<dDy<<endl;
  ofstream vxfile;
  ofstream vyfile;
  sprintf(fname,"gradx");
  vxfile.open(fname);
  sprintf(fname,"grady");
  vyfile.open(fname);
  ofstream lxfile("lapx");
  ofstream lyfile("lapy");
  for(m=0;m<M;m++){
    for(n=0;n<N;n++){
      x=(m)*hx+x1;
      y=(n)*hy+y1;
      dvx=calc_xgradient(x,y);
      dvy=calc_ygradient(x,y);
  ...
  for(t=1;t<Nit;t++){
	...
	if(y>line){
	  ...
	  add=dt*(beta/2*(Dxx*d2vx+Dyy*d2vy)-beta*beta/4*(Dxx*dvx*dvx+Dyy*dvy*dvy));//*p[m*N+n];
	  pnew[m*N+n]+=add*p[m*N+n];
	}
	//if(t%writeout==0)
	// tfile<<add<<'\t';
      }
...
double calc_xgradient(double x, double y)
{
  ...
  //vx+=9*sinx*siny;
  double dvx;
  dvx=-200*exp(-xmin1*xmin1-10*y2)*(-2*xmin1)-100*exp(-x2-10*a*a)*(-2*x)-170*exp(-6.5*b*b+11*b*c-6.5*c*c)*(-13*b+11*c)+15*exp(0.7*d*d+0.6*d*ymin1+0.7*ymin1*ymin1)*(1.4*d+0.6*ymin1);//gradx
  //  dvx+=9*2*pi*5*cos(2*pi*5*x)*siny;
...
double calc_ygradient(double x, double y)
{
  ...
  //vx+=9*sinx*siny;
  double dvy;
  dvy=-200*exp(-xmin1*xmin1-10*y2)*(-20*y)-100*exp(-x2-10*a*a)*(-20*a)-170*exp(-6.5*b*b+11*b*c-6.5*c*c)*(11*b-13*c)+15*exp(0.7*d*d+0.6*d*ymin1+0.7*ymin1*ymin1)*(0.6*d+1.4*ymin1);//gradY
  //  dvy+=9*2*pi*5*cos(2*pi*5*y)*sinx;
  return dvy;
...
void laplaceV(double x, double y, double beta, double &d2vx, double &d2vy) 
{
  ...
  //vx+=9*sinx*siny;
  double dvx;
  d2vx=-200*exp(-xmin1*xmin1-10*y2)*(4*xmin1*xmin1-2)-100*exp(-x2-10*a*a)*(4*x*x-2)-170*exp(-6.5*b*b+11*b*c-6.5*c*c)*((-13*b+11*c)*(-13*b+11*c)-13)+15*exp(0.7*d*d+0.6*d*ymin1+0.7*ymin1*ymin1)*((1.4*d+0.6*ymin1)*(1.4*d+0.6*ymin1)+1.4);//gradx
  //  d2vx-=9*4*pi*pi*25*sinx*siny;

  d2vy=-200*exp(-xmin1*xmin1-10*y2)*((-20*y)*(-20*y)-20)-100*exp(-x2-10*a*a)*((-20*a)*(-20*a)-20)-170*exp(-6.5*b*b+11*b*c-6.5*c*c)*((11*b-13*c)*(11*b-13*c)-13)+15*exp(0.7*d*d+0.6*d*ymin1+0.7*ymin1*ymin1)*((0.6*d+1.4*ymin1)*(0.6*d+1.4*ymin1)+1.4);//gradY
  //d2vy-=9*4*pi*pi*25*siny*sinx;
