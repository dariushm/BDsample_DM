...
same as pde_iterate.cpp but it does not use the symmetrized form
of the equation
...
int main(int argc, char *argv[])
{
  ...
  for(t=1;t<Nit;t++){
	...
	pnew[m*N+n]+=dt/(2*hx)*(dDx+beta*Dxx*dvx)*(p[(m+1)*N+n]-p[(m-1)*N+n]);//dpdx
	pnew[m*N+n]+=dt/(2*hy)*(dDy+beta*Dyy*dvy)*(p[m*N+n+1]-p[m*N+n-1]);//dpdy
	pnew[m*N+n]+=dt*beta*(dvx*dDx+dvy*dDy+Dxx*d2vx+Dyy*d2vy)*p[m*N+n];
      }
