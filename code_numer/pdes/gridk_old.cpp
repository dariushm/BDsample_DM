...
void gridk_r2_npole2(int i, int j, int l, double r, double r2, double theta, double st, double BC, double delR, double deltat, double Dtot, double dr2, double ka, double delpol, double delaz, int azbins, double &passoc1, double &psurvive, double ****prt, double ****prt2, double sta, double delApol, double ct, double cta, double d1, double Drot, int nsmooth, int gbins)
{
  ...
  /*az now goes from 0 to pi, so use reflecting BC.*/
  prt2[i][j][l][k]=prt[i][j][l][k]+Dtot*deltat/delR*( sfact/(r*r1mag) + derv2 )*(prt[i+1][j][l][k]-prt[i-1][j][l][k])+Dtot*deltat/dr2*derv1*derv1*(prt[i+1][j][l][k]-2.0*prt[i][j][l][k]+prt[i-1][j][l][k])+Dtot*deltat/r1sq*( ct/st/delpol*(prt[i][j+1][l][k]-prt[i][j-1][l][k]) + 1.0/delpol2*(prt[i][j+1][l][k]-2.0*prt[i][j][l][k]+prt[i][j-1][l][k]) )+ (Dtot*deltat/r1sq*1.0/(st*st*del1*del2)+ Drot*deltat/(sta*sta*del1*del2))*(prt[i][j][l][knext]-2.0*prt[i][j][l][k]+prt[i][j][l][kprev]) + Drot*deltat*( cta/(sta*delApol)*(prt[i][j][l+1][k]-prt[i][j][l][kcross]) +1.0/delApol2*(prt[i][j][l+1][k]-2.0*prt[i][j][l][k]+prt[i][j][l][kcross]) ) ;//for i=inter, j=interior, k=0
  ...
  for(n=1;n<gbins-1;n++){
    ...
    //prt2[i][j][k]=prt[i][j][k]+Dtot*deltat/(r*delR)*(prt[i+1][j][k]-prt[i-1][j][k])+Dtot*deltat/dr2*(prt[i+1][j][k]-2.0*prt[i][j][k]+prt[i-1][j][k])+Dtot*deltat/r2*(cos(theta)/st/delpol*(prt[i][j+1][k]-prt[i][j-1][k]) + 1.0/delpol2*(prt[i][j+1][k]-2.0*prt[i][j][k]+prt[i][j-1][k])+1.0/(st*st*del1*del1)*(prt[i][j][k+1]-2.0*prt[i][j][k]+prt[i][j][k-1]) );
    psurvive+=r1sq*delR*prt2[i][j][l][k]*delpol*delaz*st*delApol*sta*jac;//Is extra 2pi from other azimuth needed? and extra 2 from cutting az down to pi?
    //psurvive+=r2*delR*prt2[i][j][k]*delpol*delaz*st;
  }//done looping over az
   k=azbins-1;
  ...
  knext=kprev;
   ...
   derv2=1.0/r-sfact*sfact/(4.0*r2cube);
   jac=r/(r1mag+b/2.0);
   prt2[i][j][l][k]=prt[i][j][l][k]+Dtot*deltat/delR*( sfact/(r*r1mag) + derv2 )*(prt[i+1][j][l][k]-prt[i-1][j][l][k])+Dtot*deltat/dr2*derv1*derv1*(prt[i+1][j][l][k]-2.0*prt[i][j][l][k]+prt[i-1][j][l][k])+Dtot*deltat/r1sq*( ct/st/delpol*(prt[i][j+1][l][k]-prt[i][j-1][l][k]) + 1.0/delpol2*(prt[i][j+1][l][k]-2.0*prt[i][j][l][k]+prt[i][j-1][l][k]) )+ (Dtot*deltat/r1sq*1.0/(st*st*del1*del2)+ Drot*deltat/(sta*sta*del2*del1))*(prt[i][j][l][k-1]-2.0*prt[i][j][l][k]+prt[i][j][l][k-1]) + Drot*deltat*( cta/(sta*delApol)*(prt[i][j][l+1][k]-prt[i][j][l][kcross]) +1.0/delApol2*(prt[i][j][l+1][k]-2.0*prt[i][j][l][k]+prt[i][j][l][kcross]) ) ;//for i=inter, j=interior, k=0

   //prt2[i][j][k]=prt[i][j][k]+Dtot*deltat/(r*delR)*(prt[i+1][j][k]-prt[i-1][j][k])+Dtot*deltat/dr2*(prt[i+1][j][k]-2.0*prt[i][j][k]+prt[i-1][j][k])+Dtot*deltat/r2*(cos(theta)/st/delpol*(prt[i][j+1][k]-prt[i][j-1][k]) + 1.0/delpol2*(prt[i][j+1][k]-2.0*prt[i][j][k]+prt[i][j-1][k])+1.0/(st*st*del1*del1)*(prt[i][j][0]-2.0*prt[i][j][k]+prt[i][j][k-1]) );//for i=inter, j=interior, k=end
  psurvive+=r1sq*delR*prt2[i][j][l][k]*delpol*delaz*st*delApol*sta*jac;//Is extra 2pi from other azimuth needed? and extra 2 from cutting az down to pi?
  //psurvive+=r2*delR*prt2[i][j][k]*delpol*delaz*st;
