...
reflectin boundary conditions, symmetric version
Muller potential does not have additional roughness due to
sines
...
int main(int argc, char *argv[])
{
  ...
  for(t=1;t<Nit;t++){
    ...
    for(m=1;m<M-1;m++){
      ...
      pnew[m*N+n]+=dt*(beta/2.0*(Dxx*d2vx+Dyy*d2vy)-beta*beta/4.0*(Dxx*dvx*dvx+Dyy*dvy*dvy))*p[m*N+n];
      ...
      pnew[m*N+n]+=dt/hx2*Dxx*(p[(m+1)*N+n]+p[(m-1)*N+n]-2*p[m*N+n]);//first add in second derivative points at the old values
      pnew[m*N+n]+=dt/hy2*Dyy*(p[m*N+n-1]+p[m*N+n-1]-2*p[m*N+n]);//first add in second derivative points at the old values
      pnew[m*N+n]+=dt*(beta/2*(Dxx*d2vx+Dyy*d2vy)-beta*beta/4*(Dxx*dvx*dvx+Dyy*dvy*dvy))*p[m*N+n];
