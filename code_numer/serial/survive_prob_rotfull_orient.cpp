...
// 	ctab1/=(rab*leglen);
	...
	for(dd=0;dd<Npsibins;dd++){
	  ...
	  cout <<"direction of rotated by psi mol angle; "<<n1[0]<<' '<<n1[1]<<' '<<n1[2]<<endl;
	  //then use that vector to set target plane via normal
	  crossproduct(v, n1, pn1);//pn1 is normal to plane need to intersect.
	  cout <<"direction of normal to plane; "<<pn1[0]<<' '<<pn1[1]<<' '<<pn1[2]<<endl;
	  ...
	  cout <<"Initial separation: "<<R1<<endl;
	  cout <<"target angles: "<<costheta[ct]<<' '<<costheta[bt]<<' '<<dihed<<endl;
	  ...
	  psimol=calc_psi(bases, v, v1, n1, n2);
	  cout <<"target psi: "<<psi0<<" calculated: "<<psimol<<endl;
...
// 	    cout <<"p0 dot prod, perp 0? : "<<r0x*r1x+r0y*r1y+r0z*r1z<<endl;
// 	    cout <<"p0 dir length: "<<sqrt(r1x*r1x+r1y*r1y+r1z*r1z)<<endl;
...
// 	    cout <<"p1 dot prod, perp 0? : "<<r0x*r1x+r0y*r1y+r0z*r1z<<endl;
	  ...
	  for(i=statwrite;i<Nitbin;i+=statwrite){
	    ...
	    probfile<<tval<<' ' <<phist[ind]/(1.0*Nrep)<<' '<<1.0-passoc<<endl;
	  ...
	  tval=i*deltat_reac;
	  passoc=survive_irr( x0, tval,  Dtot,  bindrad[0],  alpha,  cof);
	  probfile<<tval<<' ' <<phist[Nwrite]/(1.0*Nrep)<<' '<<1.0-passoc<<endl;//index Nwrite is for the last time point
	  probfile.close();
	  ...
	  rfile.open(tname);
	  rfile<<0<<'\t'<<phist[Nwrite]/(1.0*Nrep)<<'\t'<<1-passoc<<'\t'<<1-passoc<<endl;  
	  sonlyfile <<costheta[ct]<<'\t'<<costheta[bt]<<'\t'<<psi0<<'\t'<<phist[Nwrite]/(1.0*Nrep)<<endl;
	  for(i=0;i<Rbins;i++){
