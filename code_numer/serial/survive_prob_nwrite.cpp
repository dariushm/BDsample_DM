  ...
  for(s=0;s<Nx0;s++){
    ...
    */
    ...
    for(i=statwrite;i<Nitbin;i+=statwrite){
      ...
      //acc[s][cnt]=1.0-passoc-phist[i]/(1.0*Nrep);
      //	cnt++;
    ...
    tval=i*deltat_reac;
    passoc=survive_irr( x0, tval,  Dtot,  bindrad[0],  alpha,  cof);
    probfile<<tval<<' ' <<phist[Nwrite]/(1.0*Nrep)<<' '<<1.0-passoc<<endl;
    //acc[s][cnt]=1.0-passoc-phist[i]/(1.0*Nrep);
    //cnt++;
    ...
    for(i=0;i<Rbins;i++){
      ...
      rfile<<R1<<'\t'<<pirrev<<'\t'<<pfree*(1-passoc)<<'\t'<<prhist[i]/(1.0*Nrep*delR*R1*R1*4.0*M_PI)<<endl;//'\t'<<pirrev<<'\t'<<pfree*(1-passoc)<<endl;
    }
    rfile.close();
  }//end over all x0s
