...
//     acc[i]=new double[Nacc];
  ...
  for(s=0;s<Nx0;s++){
	...
	probfile<<tval<<' ' <<phist[i][s]/(1.0*Nrep)<<' '<<1.0-passoc<<endl;
    ...
    rfile<<0<<'\t'<<phist[Nitbin-1][s]/(1.0*Nrep)<<'\t'<<1-passoc<<'\t'<<1-passoc<<endl;
    for(i=0;i<Rbins;i++){
  ...
  for(i=1;i<Nitbin;i++){
    ...
    if(i%statwrite==0){
      passoc=survive_irrF( x0, tval,  Dtot,  bindrad[0],  alpha,  cof);
      ratefile<<tval<<' ' <<kt0*0.5*(phist[i][0]/(1.0*Nrep)+phist[i][1]/(1.0*Nrep))<<' '<<kt0*(1.0-passoc)<<endl;
      //	acc[s][cnt]=1.0-passoc-phist[i]/(1.0*Nrep);
      //cnt++;
      //afile<<1.0-passoc-phist[i]/(1.0*Nrep)<<'\t';
    }
